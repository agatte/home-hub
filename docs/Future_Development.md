...
- **2026-06-01 — Network re-IP `192.168.1.x` → `192.168.86.0/24`** behind a self-owned Google Wifi (double-NAT). Network-wide Pi-hole DNS on the router; Google Wifi proxies DNS so per-device attribution is lost and `.lan` doesn't resolve through the router — mDNS `homehub-dashboard.local:8000` is the working hostname. Pi-hole admin `:8080` bound loopback-only; camera `/snapshot` locked to localhost+key; guest `landing_url` from `LOCAL_IP`. (Commits `31a87da`, `1f20ba7`, `1e39d8c`, `fd8769f`.)
- **2026-06-01 — Encrypted offsite backup live (restic → Backblaze B2).** Nightly `Home Hub Offsite Backup` (04:55) pushes the local backup set offsite, client-side encrypted (closes the "everything's only on the desktop, same building" gap). ARP presence/away-mode flagged for revisit post-migration (GH #96); WD My Cloud reflash idea filed (GH #97). Strategy: `~/.claude/runbooks/backup-strategy.md`.
...
Phase 4 (Game Day) core shipped 2026-05-07 (Slices A/B/C/D) + `pregameday` mode 2026-05-15 + extended scoring subtypes + dynamic volume policy. Remaining before the 2026-08-13 preseason: SEQUENCES palette/TTS iteration (GH #5, #6, #8) + preseason validation. **Custom Alexa Skill shipped 2026-05-05** (invocation `home hub` since 2026-05-16, Lambda + Cloudflare Tunnel `home-hub.gatte-home.com`, see `alexa_skill/`) — Apple Music, full autopilot, bar app remain as Phase 5.
...
**Still open:**
- Custom DNS hostname for `/guest` — **blocked** post 2026-06 Google Wifi migration (router proxies DNS; `.lan`/`.local` don't resolve through it and per-device Pi-hole attribution is lost). Guests reach the app via the QR at `http://192.168.86.210:8000/guest`; the `GET /api/guest/wifi` `landing_url` is built from `LOCAL_IP` for this reason.
- Captive-portal-style auto-redirect — infeasible without router-level DNS control. Documented in CLAUDE.md so future-Anthony doesn't relitigate.
