...
**Windows recovery reopened again — 2026-09-21:** Watching, Working, desktop-only dependencies, and supervisor restart identity handling are verified. Desktop physical presence is **not** yet closed: after the earlier 10/10 acceptance window, later continuous-at-desk sampling reproduced false `face_present=false / pose=0` intervals. Recovery remains open until the desktop camera lane is stable over a materially longer live window and across supervisor restart. The optional future offsite-backup design remains separate new infrastructure.
...
- [x] Historical recovery worktrees were retired only after proving they were clean and had zero unique commits; their branch refs remain preserved. During the reopened 2026-09-21 presence investigation, one bounded temporary worktree under `worktrees\desktop-presence-recovery` is intentionally active; `main` remains the canonical repo and unrelated Sonos/music edits in it are untouched.
...
- [x] Reloaded only the unified Windows supervisor after explicit authorization. The replacement Python 3.13 launcher/interpreter pair is live, all six retained agents report running with zero restarts, and the Scheduled Task is restored/enabled.
...
- [ ] Integrate and production-verify the now validated desktop physical-presence fallback. Longer sampling proved the residual failure is not a single-frame issue: while Anthony remained at Desk, FaceLandmarker and PoseLandmarker can both disappear for long continuous intervals.
  ...
  - The official MediaPipe Selfie Segmenter was then evaluated as **presence-only** evidence. Whole-frame mask percentage was rejected because the empty chair itself can score roughly 0.23-0.28. Spatial truth-table sampling found a much stronger distinction: seated frames had upper-half human-mask coverage >=0.0458 and central-upper coverage >=0.3041, while stable empty-chair frames had upper-half coverage 0 and central-upper coverage <=0.0837.
  - A separate 140-frame seated stress run reproduced **120 frames where both face and pose were absent**, including a **66-frame consecutive dual-miss streak**. A conservative spatial gate (upper-half >=0.015 and central upper-three-quarters >=0.15) passed **120/120 dual-miss frames** with zero failures.
  - The candidate implementation keeps FaceLandmarker as Desk/emotion authority and calibrated pose as Bed authority. Only when those cannot establish person presence may a 3-frame-qualified segmenter result publish `face_present=true` with `detection_source=segmenter`, `zone=None`, and no posture. Five consecutive negative shape frames are required to revoke segmenter presence.
  - Integrated dry-run acceptance against the real Brio: seated stress produced no shape-gate failures during dual face+pose loss; the final empty-chair run carried presence only for the bounded exit transition (1 lingering face frame + 4 segmenter-held frames), then published **15/15 consecutive absence observations** with no source/zone/posture. No frames or masks were saved or uploaded.
  - Validation before integration: **147/147** adjacent focused tests passed; broader camera/presence suite **224/224** passed; Python compile and `git diff --check` clean. Remaining gate is commit/integration plus one authorized live supervisor reload and materially longer production verification.
...
**2026-09-21 correction:** the 2026-09-20 live window and the later 10/10 acceptance window were both too short. Continuous-at-desk diagnostics subsequently reproduced long dual FaceLandmarker/PoseLandmarker outages, including a 66-frame streak, so the bounded two-frame retry in `9412a17` is retained as a useful fast-path improvement but is not sufficient as sole presence authority. The validated recovery direction adds a source-qualified, spatially gated Selfie Segmenter fallback for person presence only; it does not loosen Desk/Bed localization or emotion authority.
