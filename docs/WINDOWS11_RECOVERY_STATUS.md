...
- [ ] Prove live **desktop** Working from an actual Home Hub/Performance Audit browser session after reload. The patched classifier already produced `working / foreground_browser_work` in a live dry-run against the real project window before reload; production-row proof remains pending until that browser surface is foreground for the normal dwell.
...
- [ ] Repair `scripts/restart-agents.ps1` replacement verification for the expected Python 3.13 launcher/interpreter pair. On the authorized 2026-09-21 reload it selected launcher PID `11640` as the candidate while backend health correctly identified child interpreter PID `16656`, producing a false nonzero result even though the new supervisor was healthy; task restoration succeeded. Preserve exact native-identity safety while accepting the health-reported member of the fresh supervisor pair.
...
**2026-09-21 correction:** the 2026-09-20 live window was not sufficient to call the desktop camera lane recovered. A later continuous-at-desk reproduction showed repeated false absence. The recovered Windows 11 camera geometry still maps to the preserved Desk face-width calibration when MediaPipe locks on; the remaining defect was single-frame acquisition intermittency. The bounded two-frame retry in `9412a17` restored stable live Desk presence without loosening face/zone authority or persisting imagery.
