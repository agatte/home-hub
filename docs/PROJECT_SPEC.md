...
- **Effect auto-activation** — EFFECT_AUTO_MAP: opal (relax/day), fire (relax/night+late_night, scoped to L1+L2+L5), glisten (watching/evening+night). Every automatic mode/weather effect has an explicit reviewed L1–L5 scope; L6 never joins merely because Hue v2 discovers another mapped light. Candle was retired from auto-map 2026-05-09 (locked color state persisted through mode changes); still callable manually. Explicit manual all-light effect endpoints retain literal bridge-wide semantics and include L6. Gaming no longer auto-runs effects (they compete with screen sync and read as "RGB gamer strip").
