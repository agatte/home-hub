...
- **Effect auto-activation** — Relax is static across day/evening/night/late-night: autonomous Relax never starts a Hue dynamic effect, including weather-driven effects. Dynamic effects remain explicitly callable by the user. Watching still uses glisten in evening/night; other automatic weather effects retain reviewed L1–L5 scope. L6 never joins merely because Hue v2 discovers another mapped light. Candle remains retired from auto-map (2026-05-09). Gaming no longer auto-runs effects (they compete with screen sync and read as "RGB gamer strip").
