...
**DECIDED TARGET (#232, 2026-09-04).** `Mercury_Guest` remains Google
Guest Wi-Fi's isolated `192.168.87.0/24` visitor subnet; do not share the
Latitude/HomeHub device into it. Guest phones keep ordinary internet and the
router's Pi-hole filtering, while HomeHub access crosses only a dedicated
HTTPS guest gateway. That gateway binds loopback, uses one-use short-lived
invites plus revocable HttpOnly/Secure sessions, and forwards only visitor UI
assets and explicit `/api/guest/*` capabilities. Generic HomeHub APIs,
settings, sensing/camera, diagnostics, host controls, and ordinary WebSocket
state remain outside the guest boundary. Travel Mode suppresses the gateway.

**RESEARCH NEEDED.** Arbitrary Apple Music-to-Sonos guest requests and richer
Home Bar APIs for event-aware drink planning.
...
- **Installed fixture inventory:** L1 living-room shaded lamp; L2/L5 bedroom desk lamps; L3/L4 paired kitchen pendants; and L6 / **Plant Wash**, a Hue Play Light Bar with stable Hue/v1 ID `6`. Plant Wash is horizontal on the back edge of the rear/taller living-room plant stand, aimed upward with a slight left bias toward the couch/plant mass. Partial housing visibility is accepted because the stand frame is thin and open; couch, kitchen/island, and entry glare tests passed. Real-room calibration disproved the original 10–20% assumption: teal/blue-violet at approximately 70–75% is the successful architectural reference, saturated deep blue is too dominant for ordinary scenes, and amber/burgundy is accepted after dark. L1 remains the warm anchor in colorful scenes.
...
- **Gaming Director target** — Gaming remains one Activity; the active game is subordinate context. `GAMING_EXPERIENCE_SPEC.md` owns the detailed target: daytime prioritizes useful neutral light, weekend/evening/night progressively permit more game-specific expression, profiles are stable by default, and trustworthy game events may add only bounded reactions. Implementation is tracked by #105/#203/#204/#205.
...
- **Science-based per-mode lighting** — each mode uses distinct per-light variation: working uses CT-mode clean whites with strong task/ambient hierarchy; Gaming uses a neutral functional daytime base and time-aware game-specific HSB/CT accents under `GAMING_EXPERIENCE_SPEC.md`; Relax uses warm amber/biophilic gradients; Watching is projector-friendly and warm after sunset; Cooking uses neutral 3500K kitchen peak for accurate food color.
...
- **20 curated scenes** across 7 categories (functional, cozy, moody, vibrant, nature, entertainment, social) using color harmony theory. Existing L1–L5 values remain unchanged; L6 has an explicit conservative off target until individual presets receive real-room calibration. This explicit target also keeps effect release resolvable after a clean restart.
- **Custom scene CRUD** — user-created scenes persisted to SQLite with category and optional paired effect
- **Effect auto-activation** — Relax and Watching are static across day/evening/night/late-night: neither mode autonomously starts a Hue dynamic effect, including weather-driven effects. Dynamic effects remain explicitly callable by the user. Other automatic weather effects retain reviewed L1–L5 scope. L6 never joins merely because Hue v2 discovers another mapped light. Candle remains retired from auto-map (2026-05-09). Gaming no longer auto-runs effects (they compete with screen sync and read as "RGB gamer strip").
...
- Relax mode — "Moss & Candlelight" biophilic palette: warm ember on living-room lamps (L1/L2), muted moss/sage kitchen pendants (L3/L4), and a calibrated palette-specific architectural wash on L6. The living-room atmosphere curator owns L1/L3/L4/L6 while preserving manual per-light ownership. **Relax is static by policy:** no autonomous Hue dynamic effect runs in any time period or because of weather; explicit user-requested effects remain available. Late-night variants use the accepted amber/burgundy Plant Wash direction.
- **Screen sync for Gaming and Watching** — Desktop capture targets bedroom L2/L5. Generic Gaming deliberately ignores sampled RGB and holds its canonical period-specific state on those fixtures; Rust is the game-specific exception, holding a fixed ember color while screen luminance drives bounded brightness and the Rust event service may apply bounded damage/under-fire treatment. League champion ownership can temporarily exclude owned lights from generic ScreenSync. Watching retains dynamic RGB translation with period/zone-specific caps and bedroom-lux/weather envelope logic. L6 remains excluded from ScreenSync unless a later audited design explicitly changes its architectural role.
- **Gaming/day functional surround:** after palette/profile resolution and the ordinary mode/weather brightness composition, a brightness-only overlay protects L1 and paired L3/L4 during dark daytime conditions. At the default `1.0x` Gaming brightness, clouds floor them at `155/50`, rain at `170/70`, and thunderstorms at `180/75`; credible dark Latitude lux may lift them further within nominal `190/85` lift caps. The floors and lift caps scale with the configured Gaming brightness multiplier, preserving that user control. The overlay is strictly lift-only: it never reduces a brighter upstream state, and bright or contradictory lux never cancels a weather floor. It never changes hue/saturation or L2/L5, and does not run in evening/night/late-night.
...
- **DECIDED TARGET - 2026-09-09.** Desktop Gaming process authority requires a verified game in the foreground. A merely-running background game is context only: normal committed-mode dwell preserves a brief alt-tab, but unrelated browser/desktop input must not indefinitely renew Gaming, and strong authoritative foreground playback may displace Gaming on its normal fast Watching dwell.
...
- **DECIDED TARGET - 2026-09-09.** When YAMNet is configured, authoritative HomeHub Gaming may suspend only YAMNet inference/model state; the supervisor, microphone/RMS lane, ScreenSync, camera, display, peripherals, sleep watcher, and activity detector stay alive. Desired classifier state follows backend authoritative/manual mode, never an unaccepted local process candidate, and desktop agent health exposes desired/actual classifier state plus transition reason.
...
- **DECIDED TARGET - 2026-09-06.** Soft external-off reacquisition is a bounded Home lifecycle transaction, not generic permission for sensors to fight intentional darkness. While already Home, an explicit Auto request may release only a soft `_external_off_detected` latch when fresh trusted physical presence exists; a genuine apartment-level physical absence -> strong-presence edge may do the same only when the explicit negative observation is still inside the strong-presence freshness window. Camera startup/silence, stale or ambiguous readings, continuous presence, repeated heartbeats, and duplicate/jitter ARRIVE events while already Home do not count as reacquisition. Hard Away/RETURNING_HOME holds, Sleeping, DND, and active manual authority remain blockers. An accepted release immediately reapplies the current automatic authority with a forced resend so detector `idle` renders the Home + General/time-appropriate baseline rather than leaving the bridge dark; stronger fresh semantic activity remains eligible to refine that baseline normally.
...
- Desktop ScreenSync color picker — deterministic 8-cluster K-means with prevalence-led representative-color selection. Chromatic clusters must beat the neutral/dark share of the sampled frame before they can own the output, so tiny saturated accents cannot repaint the room; sticky bias then stabilizes genuinely chromatic scenes.
...
- **No sidebar** — floating glassmorphic bottom pill bar (Home, Music, Analytics, Settings) + mode overlay (Bebas Neue 36px all-caps mode name with character-stagger animation) + Now Playing chip that opens the global music-player overlay
...
- Curated blocklists cover ads, malware, phishing, tracking, and Windows telemetry. After the 2026-09-04 HaGeZi legacy-feed cleanup, gravity contained about 785k effective unique blocked domains; counts are upstream-dependent and change over time. HaGeZi TIF remains disabled because it caused false positives on email click-trackers. A 191-domain anudeepND functionality allow-adlist is active (antigravity source).
...
- Network-wide DNS via the personal Google Wifi router (post-2026-06 migration; no per-device config needed). Google Wifi proxies DNS, so clients see `.86.1` and per-device Pi-hole attribution is lost. As accepted under #229 on 2026-09-04, Google Wifi uses Pi-hole `192.168.86.210` as its primary upstream DNS and `8.8.8.8` as the independent secondary fallback.
- **Google Guest Wi-Fi (#232, 2026-09-04):** `Mercury_Guest` is enabled and a real iPhone received `192.168.87.20` / gateway `192.168.87.1`, proving a distinct `192.168.87.0/24` guest subnet. Normal internet worked, a known Pi-hole-blocked hostname remained blocked, and both `192.168.86.210:8000/guest` and the main HomeHub surface were unreachable with no Google Shared Devices configured. Preserve that isolation; guest HomeHub access uses the dedicated HTTPS gateway above rather than sharing the Latitude.
- **Always-home reliability checkpoint (#145, 2026-09-04):** the Windows desktop currently receives only `192.168.86.1` as Wi-Fi DNS. The 2026-09-02 real-away test showed that this desktop still resolved through Google Wifi while the Latitude/backend was physically absent. A controlled 2026-09-04 Pi-hole outage then proved the same path with direct evidence: `.210` timed out while `192.168.86.1` resolved four unique cache-miss names, ordinary system DNS still resolved after a client-cache flush, and HTTPS returned 200. This disproves the old assumption that Latitude/Pi-hole removal necessarily kills DNS for this client, but it does **not** prove router-wide behavior for every apartment device. A second controlled stop used Anthony's iPhone on Project Mercury with cellular data and iCloud Private Relay disabled; fresh Wikipedia and Python.org loads both succeeded while Pi-hole was confirmed down. Pi-hole was restored immediately. The ordinary apartment DNS fail-safe acceptance was satisfied and #145 is closed; #229 later restored Pi-hole as Google Wifi’s primary upstream while retaining the independent public-DNS fallback.
...
- ~~Hard to read at a glance~~ — fixed: mode overlay with large mode name, weather widget, Now Playing chip + global player overlay
...
- ~~Weather-Reactive Lighting~~ — shipped: noticeable light adjustments across all active modes (thunderstorm=purple+sparkle, rain=cool, golden_hour=warm, overcast=dim+warm, snow=bright+cool+opal). Candle was removed from the rain overlay 2026-05-09 alongside its retirement from EFFECT_AUTO_MAP. NWS alert override ensures storms are detected even when observation stations lag. Weather-driven music suggestions broadcast via WebSocket during rain/storm/snow
- ~~Weather-Reactive Ambient Sound~~ — shipped: `AmbientSoundService` auto-plays a matching ambient bed on Sonos when weather class changes. Browser `<audio>` fallback was removed in July 2026; ambient sound is Sonos-only and fails silent when Sonos is unavailable or busy. Manual pause/stop of auto-started ambient learns the rejection by disabling the current mode auto-play or weather-reactive ambient. 2026-05-26 upgrade adds **internet-radio nature streams** as the preferred source over the committed loop files: a curated `ambient_streams` app_setting (`{wclass: [{id, label, url}]}`, seeded from `DEFAULT_STREAM_LIBRARY`) plays via Sonos `play_uri(force_radio=True)`. A background health probe (2xx + `audio/*`, 5-min TTL cache, off the mode-change hot path) picks a live stream; if none is healthy the committed loop file (`rain.mp3`, …) is the automatic fallback. SSRF-safe: only the exact curated stream URLs are registered to the Sonos play_uri allowlist. Classes without a reliable 24/7 stream (thunderstorm/snow/wind/fireplace) fall back to files. **Per-class source as of 2026-05-26:** rain = live stream (*Nature Radio Rain*) → file fallback; thunderstorm / wind / snow = curated long-form loops; `nature` = manual-pick stream (*Real World Sounds*). The thunderstorm/wind/snow loops are **Public-Domain field recordings hand-placed in `data/ambient/` on the Latitude** (gitignored — not in the repo, re-fetchable per memory `project_ambient_weather_loop_sources`): thunderstorm = 25-min rain+thunder, wind+snow = a shared "Cold Winter Wind" bed (snow is silent → its real ambience is winter wind). These shadow the short committed loops via the `data/ambient/` > `static/ambient/` scan priority. No dedicated thunderstorm/wind/snow internet-radio stream exists — verified 2026-05-26 across internet-radio.com, SHOUTcast, rcast, TuneIn (rain+thunder hits were podcasts or subscription-gated)
...
| POST | `/api/personality/vibe` | Natural-language vibe request `{transcript, timing}`. `timing="arrival_if_away"` applies immediately when home or stages to `app_settings.pending_arrival_vibe` while away; after a geofence-arrive event, `AwayManager._on_arrive()` applies and clears the staged vibe. Trusted camera observations reconcile occupancy through `AwayManager`; `AutomationEngine.signal_presence(...)` remains the runtime suppression-release mechanism behind accepted lifecycle decisions. Deterministic rules map common phrases to existing modes/curated scenes first; optional Anthropic fallback is validated against known modes/scenes. Tunnel callers require both `X-API-Key` and `X-Skill-Token`; iOS Shortcut source is `X-Source: ios_shortcut:vibe` |
...
| `set_effect_all` | `(effect: str) → bool` | Apply to every dynamically mapped bridge light (currently L1–L6) |
| `stop_effect` | `(v1_light_id: str) → bool` | Stop on one light |
| `stop_effect_all` | `() → bool` | Stop effects on every dynamically mapped bridge light (currently L1–L6) |
...
Latitude-side watchdog over the desktop agent-supervisor's heartbeats (POSTed to `/api/automation/agent-health` every ~30s). Closes the gap the in-supervisor self-heal can't cover — the supervisor going silent *entirely* (process crash, desktop asleep/off, network drop), which on 2026-06-03 degraded desktop presence for hours with no alert. A 60s `poll_loop` fires a DND-respecting `notifier.emit_alert` when the supervisor has been silent > 5 min, or when an individual agent stays `stopped`/`hung` > 3 min (the supervisor's own restart isn't clearing it). Alerts are edge-triggered — one per incident plus a recovery notice — and suppressed while the desktop is *expected* offline (`current_mode == "sleeping"` or the apartment-empty `_external_off_detected` state); DND gates the phone push. `GET /api/automation/agent-health` returns a `watchdog` freshness snapshot (`online`, `silent_for_seconds`, `stuck_agents`, …) for the check-back sweep + deploy-verifier. The watchdog registers its own `agent_health_monitor` heartbeat (60s) in the `HeartbeatRegistry`, so a hung watchdog is itself surfaced as stale in `/health`. `GET /api/automation/agent-health` stores reports by origin so Latitude agents do not overwrite desktop-supervisor health.

#### LatitudeStreamingDetector (shipped 2026-07-04; authority retraction hardened 2026-09-05)
Standalone systemd user service on the Latitude (`home-hub-latitude-streaming.service`). Polls MPRIS and PipeWire via `gdbus`/`wpctl`; reports `mode=watching`, `source=process`, `device=latitude` only when media playback is active. A merely open Stremio window is not enough. On playback stop, reports `idle` after a short dwell so brief pauses/buffering do not churn the room. Health posts to `/api/automation/agent-health` with `origin=latitude` so it does not overwrite or reset desktop-supervisor watchdog state.

**Authority retraction contract.** Latitude `idle` is a device-scoped retraction of positive media intent, never global apartment Idle. If `process:latitude` owns raw authoritative Watching when that accepted semantic retracts, AutomationEngine immediately hands raw automatic authority to the highest-priority remaining fresh accepted process semantic, using recency as the established tiebreaker. If none remains, raw `idle` projects to House=`Home` + Activity=`General` and time-of-day lighting. Stale remaining semantics are ineligible. The retracted Latitude owner/freshness stamp cannot hold another source behind `SOURCE_STALE_SECONDS`, repeated retractions are idempotent, and periodic reconciliation cannot reapply ownerless Watching. Manual overrides, Sleeping, DND, Away/RETURNING_HOME, external-off suppression, and other protected ownership keep their authority; the stale underlying process owner is still cleaned so it cannot reappear when protection later releases. ConfidenceFusion remains shadow-only.
...
- ✓ **Now Playing chip + player overlay** — Fixed bottom-right chip shows album art + track, pulses when playing, and opens `MusicPlayerOverlay.svelte` in-place. The overlay blurs/dims the current page, reuses the Sonos store + `sonosCommand` path for previous/play-pause/next/volume, supports Escape/backdrop/close dismissal, and keeps `/music` as the deeper discovery/settings page via an "Open Music" link.
- ✓ **Plant app widget** — polls the external Vercel-hosted plant care app, shows total / needs-water / overdue counts + next watering. Tapping anywhere on the Plants card opens the full plant app inside a fullscreen iframe modal layered over the dashboard (no new tab — the kiosk Firefox stays on the dashboard).
...
- ✓ **Effect policy** — Relax and Watching remain static in every period; manual effect commands remain available. Party scenes use static palettes only (bridge effects flatten per-light HSB to their own color base — incompatible with custom palettes).
...
- **Plant app widget** — Shows live status from the plant tracking web app (needs water count, next care action). Animated card on dashboard. Tapping anywhere on the Plants card opens the full plant app in a fullscreen iframe modal portaled to `document.body` — the dashboard SPA stays loaded underneath, and a fixed close button (plus ESC and backdrop click) always returns to it. This avoids the kiosk-Firefox lockout that `target="_blank"` would cause.
...
- ✓ **Living Ink frontend redesign** — generative canvas background (Perlin noise, data-reactive to Hue lights + Sonos), glassmorphic cards, Bebas Neue typography, mode overlay with character-stagger animation, Now Playing chip + global player overlay, 60s auto-hide on idle
...
- **Google Wifi DNS model (post-2026-06 migration)** — the apartment runs behind a self-owned Google Wifi (`192.168.86.0/24`, double-NAT). Google Wifi proxies DNS, so clients see `.86.1` and per-device Pi-hole attribution is lost. Accepted #229 topology (2026-09-04): primary upstream DNS is Pi-hole `192.168.86.210`; secondary is `8.8.8.8`. Repeated router-path tests matched Pi-hole blocking with 0/20 sampled secondary leaks while Pi-hole was healthy; a controlled Pi-hole outage then proved fresh DNS continued through the secondary resolver, and filtering resumed after recovery. The router answers `.lan` with authoritative NXDOMAIN, so `.lan` names resolve only for clients pointed directly at Pi-hole. The working zero-config hostname is mDNS (`homehub-dashboard.local:8000`, avahi on the Latitude); never put `.local` records in Pi-hole. On iPhone/Safari, iCloud Private Relay can bypass local DNS; for Project Mercury, disable the per-network **Limit IP Address Tracking** setting when Pi-hole filtering is desired.
- **Google Guest Wi-Fi boundary (#232)** ? `Mercury_Guest` is isolated on `192.168.87.0/24` and still inherits router/Pi-hole DNS filtering. Do not add the Latitude as a Google Shared Device. Guest app reachability is intentionally internet-mediated through the separate loopback guest gateway and one-use invitation/session flow; the Settings Network & Guest Access surface must distinguish local gateway health from successful public Cloudflare/DNS reachability before reporting the guest app ready or minting a QR.
- **Pi-hole on same machine** — Pi-hole Docker runs on the Latitude alongside Home Hub. Direct Pi-hole clients lose that resolver when the Latitude/Docker stack is absent, but Google Wifi now uses `192.168.86.210` only as its primary upstream and keeps `8.8.8.8` as independent fallback. Controlled desktop and iPhone outage tests plus the #229 primary/fallback validation proved ordinary DNS survives Pi-hole loss and filtering resumes after recovery.
