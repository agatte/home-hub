...
- **Effect auto-activation** — EFFECT_AUTO_MAP: opal (relax/day), fire (relax/night+late_night, scoped to L1+L2 so moss pendants stay static), glisten (watching/evening+night). Candle was retired from auto-map 2026-05-09 (locked color state persisted through mode changes); still callable manually. Gaming no longer auto-runs effects (they compete with screen sync and read as "RGB gamer strip").
