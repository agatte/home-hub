...
**Viewer-deferred scoring TTS preparation (2026-09-21 hardening):** for trusted Colts scoring events that are already waiting on the Hulu viewer clock and have spoken commentary, CelebrationOrchestrator selects the exact eventual line and asks TTSService to generate the MP3 during that wait. Preparation is file/network work only: it acquires no Sonos ownership, snapshots nothing, and performs no volume/transport mutation. At viewer release, lights start immediately and the TTS branch rechecks normal Game Day/DND/audio authority before consuming the prepared file through the existing ownership-gated `speak()` transaction. Preparation failure/timeout falls back to on-demand generation; seek-drop, authority loss, cooldown suppression, shutdown, or other non-consumption discards the prepared file. PAT remains lights-only; timestamp-untrusted embedded conversions never enter this path.
...
- **Agent fleet wiring** — ✓ shipped 2026-05-07. `gameday-preflight` fires from runbook entry #12 (preseason T-7) and #13 (weekly Sunday Aug-Jan) with bye-week early-exit. `gameday-postmortem` previously auto-fired from the retired Claude loop pre-fire detector when a `gameday:auto` window closed. As of 2026-07-13, use Codex `$homehub-diagnose` plus manual postmortem checks instead. See `docs/AGENT_STRATEGY.md` Part 5 for the historical T-7d → T+90 timeline.
...
Canonical Game Day provider state remains real-time and must never be delayed to match a viewer stream. User-facing Game Day surfaces share one presentation timeline: the stadium display, current-drive/clock presentation, celebration flair, lights, and TTS must not reveal an ESPN event before the trusted viewer media clock reaches the **translated viewer-program time** for that event. ESPN event wallclock and Hulu/Chromium MPRIS program time are different clock domains; direct timestamp equality is not a valid synchronization contract.

On 2026-09-15, Hulu Live in Chrome/Chromium on Latitude was physically validated through MPRIS `Position`: steady "live" playback measured about 30.5 seconds behind wall clock, a ~15 second pause increased lag to about 49.2 seconds, and Hulu "Jump to Live" returned it to about 28.45 seconds. Chrome's clock advanced smoothly with sub-second precision. Firefox exposed an epoch-like position too, but showed periods where `PlaybackStatus=Playing` while position stopped advancing, so Firefox is telemetry/fallback only until separately proven reliable.

The 2026-09-20 Colts @ Chiefs real-room session established a second, separate calibration: the same visible football moment maps to Hulu/Chromium program time at approximately **ESPN provider wallclock + 29.0 seconds**. This is **not** an added stream delay and must not be implemented as a celebration sleep. It is a source-specific clock-domain translation applied only by the Hulu/Chromium viewer-sync layer. The value is runtime-configurable/persisted as the Hulu program-time offset; non-Hulu or lower-latency sources do not inherit it.

Chrome/Hulu MPRIS `Position` is the preferred adaptive viewer clock. `mpris:length` is not authority; Chromium exposes `INT64_MAX` for live/indefinite media and HomeHub normalizes that sentinel to unknown. Viewer-clock authority requires a plausible epoch timestamp, fresh samples, Chrome/Chromium + Hulu identity, and consecutive ~1x advancement; a real Paused status may remain trusted and hold the viewer timeline.

Kickoff uses provider play time when available (scheduled kickoff otherwise). In-progress provider-time frames/events receive the Hulu program-time translation before comparison with MPRIS. ESPN does not expose a distinct trustworthy final-whistle wallclock, so final presentation/effects use the first provider observation of `final` **without** the provider-to-program translation; this intentionally prefers a bounded late release over reusing an already-buffered last-play timestamp and risking an early or suppressed final frame.

Canonical `gameday_state` is always published first. A separate `gameday_viewer_state` is selected from bounded in-memory state history according to the trusted media timestamp after the appropriate clock-domain translation. A score-changing viewer frame is queued only when newly extracted, independently timed scoring events account for the complete provider score delta. If ESPN exposes a score before a trustworthy play wallclock, canonical truth still updates immediately but the viewer frame and room effects wait; observation/poll time must never be substituted as provider media time. Quarter/OT transitions are the bounded exception because ESPN may advance `status.period` before a newer drive play exists: when the score is unchanged and no genuinely newer trusted play anchors the new period, the viewer frame uses the first provider observation time **without** the +29 provider-play translation. This prevents a stale quarter label while preserving score no-spoiler gates and monotonic viewer ordering. While sync is active, both Game Day presentation routes consume only that viewer state; after restart, route re-entry, rewind, or clock-confidence reacquisition, they show a truthful "SYNCING TO LIVE TV" no-spoiler state until a safe viewer frame is available rather than exposing canonical future data.

A large forward seek such as "Jump to Live" advances presentation state but drops celebrations for events deliberately skipped over; skip ranges are evaluated in translated viewer-program time so the same clock contract governs both waiting and seek suppression. Brief seek-confidence reacquisition holds the last synchronized presentation frame. If the viewer clock becomes stale, unsupported, disabled, or otherwise untrustworthy, deferred effects fall back through normal Game Day authority checks rather than remaining stuck. Viewer sync must remain observable and runtime-disableable, and it must never weaken final/manual/scene/ScreenSync/lighting ownership gates.

ESPN touchdown rows may later mutate to include a successful PAT/two-point conversion while retaining the parent touchdown wallclock. Such an embedded conversion has no independent trustworthy presentation timestamp. HomeHub may retain that conversion for canonical score/history truth, but it must not synthesize a separately timed room effect from the touchdown timestamp; a standalone conversion row with its own trustworthy provider wallclock remains eligible.
