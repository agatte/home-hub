...
Kickoff uses provider play time when available (scheduled kickoff otherwise). ESPN does not expose a distinct trustworthy final-whistle wallclock, so final presentation/effects use the first provider observation of `final`; this intentionally prefers a bounded late release over reusing an already-buffered last-play timestamp and risking an early or suppressed final frame.

Canonical `gameday_state` is always published first. A separate `gameday_viewer_state` is selected from bounded in-memory state history according to the trusted media timestamp. While sync is active, both Game Day presentation routes consume only that viewer state; after restart, route re-entry, rewind, or clock-confidence reacquisition, they show a truthful "SYNCING TO LIVE TV" no-spoiler state until a safe viewer frame is available rather than exposing canonical future data.

A large forward seek such as "Jump to Live" advances presentation state but drops celebrations for events deliberately skipped over; it must never dump queued lights/TTS afterward. Brief seek-confidence reacquisition holds the last synchronized presentation frame. If the viewer clock becomes stale, unsupported, disabled, or otherwise untrustworthy, deferred effects fall back through normal Game Day authority checks rather than remaining stuck. Viewer sync must remain observable and runtime-disableable, and it must never weaken final/manual/scene/ScreenSync/lighting ownership gates.
