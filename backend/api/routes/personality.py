...
class BlendshapeSubmit(BaseModel):
    ...
    # Note: FaceLandmarker has no top-level detector score. For the desktop
    # source, native FaceLandmarker success uses 1.0; when the full-range
    # BlazeFace fallback supplies presence, its detector score is sent instead.
    # Latitude has its own detector semantics, so do not compare magnitudes
    # across sources for confidence-weighted logic.
