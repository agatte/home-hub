...
async def test_speak_restores_volume_when_play_uri_fails(tmp_path: Path) -> None:
    ...
    sonos.get_current_playback_snapshot = AsyncMock(
        return_value={"uri": "x-rincon:fake", "position": "0:01:23", "volume": 25}
    )
...
async def test_speak_restores_volume_when_play_uri_raises(tmp_path: Path) -> None:
    ...
    sonos.connected = True
    sonos.get_current_playback_snapshot = AsyncMock(
        return_value={"uri": "x-rincon:fake", "position": "0:00:30", "volume": 18}
    )
...
async def test_speak_succeeds_and_still_restores(tmp_path: Path) -> None:
    ...
    sonos.connected = True
    sonos.get_current_playback_snapshot = AsyncMock(
        return_value={"uri": "x-rincon:test", "position": "0:00:00", "volume": 22}
    )
...
@pytest.mark.asyncio
async def test_speak_skips_restore_when_nothing_was_playing(tmp_path: Path) -> None:
    """No prior playback (snapshot=None) → no restore_playback call,
    but volume is still reset (since we may have bumped it pre-play)."""
    sonos = AsyncMock()
    sonos.connected = True
    ...
    # Volume restored to the value captured before play_uri
    sonos.set_volume.assert_called_with(30)
    # No prior playback → no restore_playback
