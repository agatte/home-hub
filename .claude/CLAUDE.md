...
- **`pc_agent/ambient_monitor.py`** — Blue Yeti RMS + YAMNet. RMS produces only "idle" edge + heartbeat. **Social is YAMNet-gated** (`speech_multiple` ≥0.80 sustained 30s); `--shadow` mode → social manual-only. Never records audio.
