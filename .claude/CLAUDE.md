...
- **`automation_engine.py`** — `_evaluate_zone_posture_rule` is env-gated by `ZONE_POSTURE_RULE_APPLY` (set false to shadow-log only). Late-night rescue + ambient_relax + zone+posture rule + both attendance vetoes + `_is_away` veto live in `run_loop`. Mode priority: gameday(6) > gaming(5) > social(4) > watching(3) > working(2) > idle(1) > away(0.5) > sleeping(0). Away mode is set only by `PresenceService` (no sensor lane reports it organically).
...
| `idle` | No process detected, OR Win32 idle >10min, OR camera absent ≥30s | Falls through to time-based rules. After 180s of continuous idle (no Sonos, no fresh desk/process attendance, not away), `ambient_relax` autonomous setter pushes to `relax` as the soft default |
| `away` | `PresenceService` polls the Latitude's ARP table for `PRESENCE_PHONE_IP` every 30s with a UDP-5353 nudge. 5 consecutive misses → away; 1 hit → home. Requires iOS "Private WiFi Address → Fixed" | All lights off. Sonos auto-pauses. Vetoes late_night_rescue, ambient_relax, zone_posture_rule, watching_sleep_guard. Auto-clears when iPhone reappears on the LAN — pre-departure manual brightness stamps preserved |
