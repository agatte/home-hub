...
- **`pc_agent/activity_detector.py`** — gaming uses `GAME_PROCESSES` plus cached Steam library manifest executable discovery; `javaw.exe` stays excluded (JetBrains FPs). Media foreground-gated. LoL champion resolved off `/liveclientdata/allgamedata` via `_resolve_active_champion` cross-walking `activePlayer.riotId` → `allPlayers` roster (fallback `summonerName` for spectator/replay).
...
| Automation | `/api/automation` | `automation.py` — mode status/override, schedule, brightness mult, social styles, screen sync, mode→scene, DND, watching-posture + `rust-lighting` (GET/PUT live Rust luma-bri envelope tuning → `rust_lighting_config`) + `rust-event` (POST damage-vignette ingest → L2/L5 flinch + under-fire glow) / `rust-event-config` (GET/PUT feel knobs → `rust_event_config`); both no-redeploy |
...
| `gaming` | Game binaries in `game_list.py` plus cached Steam installed-game executable discovery (NOT `javaw.exe` — JetBrains FP) | Neutral fill + blue/purple peripheral accents, warm desk-lamp bias. Night: deep blue ambient. Screen sync on L2, glisten eve/night. In an active League match, L2 + L5 shift to the champion's color (`LoLChampionService` → `champion_color_map`, bypassing screen-sync on those lamps) |
...
| `idle` | No process detected, OR Win32 idle >10min, OR camera absent ≥30s | Falls through to time-based rules. After 600s of continuous idle (no Sonos, no recent desk/process attendance), `ambient_relax` autonomous setter pushes to `relax` as the soft default |
