...
| `away` | Hue Bridge geofence reports all configured phones absent (`HueGeofenceService` polls `/clip/v2/resource/geofence_client` every 60s) | All lights off. Sonos auto-pauses. Vetoes late_night_rescue, ambient_relax, zone_posture_rule, watching_sleep_guard. Auto-clears when any phone returns home — pre-departure manual brightness stamps preserved |
