...
echo "  2. Set the router's network-wide DNS to 192.168.86.210 (own Google"
echo "     Wifi as of 2026-06). NOTE: Google Wifi proxies DNS — clients see"
echo "     the router (.86.1), so per-device Pi-hole attribution is lost and"
echo "     local .lan records don't resolve through the router. Pi-hole's"
echo "     upstream is the Unbound recursive + DNSSEC resolver alongside it."
...
echo "  4. Restart Home Hub on the Latitude: systemctl --user restart home-hub.service"
